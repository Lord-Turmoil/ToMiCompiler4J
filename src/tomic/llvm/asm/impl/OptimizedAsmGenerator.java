...
public class OptimizedAsmGenerator implements IAsmGenerator, IAstVisitor {
    ...
    private AllocaInst parseArrayDef(SyntaxNode node) {
        ...
        initArray(array, null, null, expNodes, 0);

        return array;
    }

    private void initArray(Value base, ArrayType type, List<Integer> prefix, List<SyntaxNode> initValues, int offset) {
        if (type == null) {
        ...
        if (type.getElementType() instanceof ArrayType elementType) {
            int size = type.getElementType().getBytes() / 4;
            for (int i = 0; i < type.getElementCount(); i++) {
                var subscripts = new ArrayList<>(prefix);
                subscripts.add(i);
                initArray(base, elementType, subscripts, initValues, offset + i * size);
            }
        } else {
            for (int i = 0; i < type.getElementCount(); i++) {
                var subscripts = buildSubscript(prefix, i);
                var inst = GetElementPtrInst.create(base, subscripts);
                var initValue = initValues.get(offset + i);
                initArrayElement(inst, initValue);
            }
        }
    }

    private void initArrayElement(GetElementPtrInst inst, SyntaxNode initValue) {
        insertInstruction(inst);
        var value = ensureInt32(parseExpression(initValue));
        insertInstruction(new StoreInst(value, inst));
    }

    private List<Value> buildSubscript(List<Integer> prefix, int index) {
        var context = module.getContext();
        ArrayList<Value> subscripts = new ArrayList<>();
        subscripts.add(new ConstantData(IntegerType.get(context, 32), 0));
        for (var i : prefix) {
            subscripts.add(new ConstantData(IntegerType.get(context, 32), i));
        }
        subscripts.add(new ConstantData(IntegerType.get(context, 32), index));
        return subscripts;
    }

    private void _initArray(Value base, List<SyntaxNode> initValues, int offset) {
        Type arrayType = base.getPointerType().getElementType();
        ...
        if (type.getElementType().isIntegerTy()) {
            ...
            var inst = GetElementPtrInst.create(base, List.of(
                    ...
                    new ConstantData(IntegerType.get(module.getContext(), 32), 0)));
            insertInstruction(inst);
            _initArray(inst, initValues, offset);
            ...
            for (int i = 1; i < type.getElementCount(); i++) {
                ...
                insertInstruction(inst);
                _initArray(inst, initValues, offset + i * size);
            }
        }
    }
